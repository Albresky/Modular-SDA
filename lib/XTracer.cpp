...
XTracer::XTracer(QCustomPlot* _plot, TracerType _type, QObject* parent)
    : QObject(parent),
      m_type(_type),
      m_plot(_plot)
{
...
void XTracer::setPen(const QPen& pen)
{
    if(m_tracer)
    {
        m_tracer->setPen(pen);
...
void XTracer::setBrush(const QBrush& brush)
{
    if(m_tracer)
    {
...
void XTracer::setLabelPen(const QPen& pen)
{
    if(m_label)
    {
        m_label->setPen(pen);
...
void XTracer::setText(const QString& text)
{
    if(m_label)
    {
...
void XTracer::setVisible(bool vis)
{
    m_visible = vis;
    if(m_tracer)
    {
...
void XTracer::updatePosition(double xValue, double yValue)
{
    if (!m_visible)
    {
        setVisible(true);
...
XTraceLine::XTraceLine(QCustomPlot* _plot, LineType _type, QObject* parent)
    : QObject(parent),
      m_type(_type),
      m_plot(_plot)
{
...
void XTraceLine::updatePosition(double xValue, double yValue)
{
    if(VerticalLine == m_type || Both == m_type)
    {
        if(m_lineV)
